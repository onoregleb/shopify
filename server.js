...
(() => {
  ...
  // Define the backend URL for usage tracking (set window.__VTON_APP_URL before this script if needed)
  const APP_BACKEND_URL = window.__VTON_APP_URL || "https://localhost:3458"; 
  ...
  async function processImage() {
    ...
    try {
      ...
      // Списываем кредит за примерку
      try {
        // ВАЖНО: отправляем запрос на внутренний endpoint, а не на внешний API
